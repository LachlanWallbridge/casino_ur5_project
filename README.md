...
- [2. Project Overview](#2-project-overview)
  - [2.1 Problem Statement and Customer](#21-problem-statement-and-customer)
  - [2.2 Robot Functionality Summary](#22-robot-functionality-summary)
  - [2.3 Demo Video](#23-demo-video)
    - [2.3.1 Initial Testing Video](#231-initial-testing-video)
    - [2.3.2 Presentation Day Video](#232-presentation-day-video)
- [3. System Architecture](#3-system-architecture)
  - [3.1 ROS2 Node Graph](#31-ros2-node-graph)
  - [3.2 Package-Level Architecture](#32-package-level-architecture)
  - [3.3 Behaviour Tree / State Machine](#33-behaviour-tree--state-machine)
  - [3.4 Node Summaries](#34-node-summaries)
  - [3.5 Custom Messages and Interfaces](#35-custom-messages-and-interfaces)
    - [3.5.1 Custom Messages](#351-custom-messages)
    - [3.5.2 Custom Actions](#352-custom-actions)
    - [3.5.3 Custom Services](#353-custom-services)
- [4. Technical Components](#4-technical-components)
  - [4.1 Computer Vision Pipeline](#41-computer-vision-pipeline)
    - [4.1.1 ArUco Board Detection (`aruco_cv`)](#411-aruco-board-detection-aruco_cv)
    - [4.1.2 Dice Detection (`dice_cv`)](#412-dice-detection-dice_cv)
    - [4.1.3 Cup Detection (`cup_cv`)](#413-cup-detection-cup_cv)
    - [4.1.4 Player and Bet Detection (`player_node`)](#414-player-and-bet-detection-player_node)
  - [4.2 Custom End-Effector](#42-custom-end-effector)
  - [4.3 System Visualisation](#43-system-visualisation)
    - [4.3.1 RViz Visualisation](#431-rviz-visualisation)
    - [4.3.2 Web Dashboard Visualisation](#432-web-dashboard-visualisation)
  - [4.4 Closed-Loop Operation](#44-closed-loop-operation)
- [5. Installation and Setup](#5-installation-and-setup)
  - [5.1 Requirements](#51-requirements)
  - [5.2 Workspace Setup and Cloning the Repo](#52-workspace-setup-and-cloning-the-repo)
  - [5.3 Hardware Setup](#53-hardware-setup)
    - [UR5e Startup](#ur5e-startup)
    - [UR5e Connection](#ur5e-connection)
    - [Camera Setup](#camera-setup)
    - [End-Effector Installation](#end-effector-installation)
    - [Teensy Connection](#teensy-connection)
  - [5.4 Configuration and Calibration](#54-configuration-and-calibration)
- [6. Running the System](#6-running-the-system)
  - [6.1 One-Command Launch](#61-one-command-launch)
  - [6.2 Example Commands](#62-example-commands)
  - [6.3 Expected Behaviour](#63-expected-behaviour)
...
- [8. Discussion and Future Work](#8-discussion-and-future-work)
  - [8.1 Challenges and Solutions](#81-challenges-and-solutions)
  - [8.2 Potential Improvements](#82-potential-improvements)
  - [8.3 Project Novelty](#83-project-novelty)
...
- [11. References and Acknowledgements](#11-references-and-acknowledgements)
  ...
  - [11.2 Acknowledgements](#112-acknowledgements)
...
A major engineering challenge was understanding and controlling MoveIt’s OMPL-based global planner. The planner often produced unpredictable or inefficient trajectories, causing the robot to make unnecessary spins, large detours or motions that did not match the intended task. Debugging this was difficult because MoveIt provides very little visibility into OMPL’s internal sampling, making it hard to understand why a particular path was selected. This unpredictability made precise manipulation tasks such as picking dice or flipping the cup unreliable.

The issue was resolved by replacing OMPL-based planning with a custom Cartesian planner, which is deterministic, easy to constrain and far more predictable. Cartesian paths ensure that the robot moves smoothly and logically, making trajectory generation simple, fast and well suited to the task.

### Service Callback Blocking MoveGroup Updates → Solved by Switching to an Action Server
Another major issue appeared in the moveit_path_planner package, where the Cartesian planner was originally implemented as a service server. Because ROS 2 services are synchronous and blocking, the service callback prevented MoveGroup from updating the robot’s internal state during planning. As a result, the planner often used stale robot states, which led to incorrect trajectories, planning failures or unsafe motions. The root cause was that both MoveGroup and the planner were running on the same callback thread, limiting MoveGroup’s ability to update joint states while planning.

The fix was to convert the planner into an Action Server, which runs in its own threads and supports non-blocking execution. This allowed MoveGroup to update robot states continuously during planning and execution, fully removing the stale-state issue and making the planning pipeline stable and reliable.

### Callback Deadlocks and Concurrency Problems in the Brain Node
The Brain node initially suffered from callback deadlocks and unsafe motion sequencing because subscriptions, services, the ActionClient and the round thread were all competing for a single executor thread. Callbacks frequently blocked one another, stopping perception updates and action feedback and causing the system to freeze mid-round.

This was fixed by introducing a ReentrantCallbackGroup and a MultiThreadedExecutor so callbacks could run concurrently, and by using threading.Event() to synchronise robot motions. These changes ensured that actions executed safely in sequence while perception and other callbacks continued to run in parallel, removing deadlocks and making motion execution deterministic and reliable.
